...
def resample(samples, sampling_rate, universal_sr=44100):
...
def make_spectrogram(samples, sampling_rate=44100, seconds=5, window_width=512, spectrogram_type="normal", verbose=False):
...
def load_spectrograms(file_path, normalize_samples=False, filter_noise=False):
    """
        :param file_path:             str, file path spectograms should be loaded from
        :param normalize_samples:    bool, whether audio should be normalized before made into spectograms
        :param filter_noise:         bool, whether noise should be filtered from audio before creating spectograms
        :return:                     numpy.array,     A 3d array that contains the spectrogram slices. If the sound does not have a duration that is dividable by 5,
                                                    the last few seconds will be lost.
    """
    ...
    #Filter noise from samples (does not function very well yet on test data)
    if filter_noise:
        samples = extract_noise(samples, sampling_rate, verbose=False)
